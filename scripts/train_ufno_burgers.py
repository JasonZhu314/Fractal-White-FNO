"""Train U-FNO (1D) on 1D Burgers' equation.
...
Usage:
    python scripts/train_ufno_burgers.py
"""
...
###################################
# Configuration
# Same resolution/data as FNO script for fair comparison
###################################
...
###################################
# Construct model (scaled to match FNO paper: k_max=16, width=64)
###################################
k_max = 16
n_fno_layers = 3
n_ufno_layers = 3
...
print(f"UFNO1d parameters: {model.count_params():,}")

###################################
# Output directory
###################################
output_dir = os.path.join(os.path.dirname(__file__), '..', 'outputs', 'ufno_burgers')
...
train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = FNO_train(
    ...
    save_model_name=os.path.join(output_dir, "UFNO_burgers_model")
)
...
# Plot results
###################################
plot_losses(train_rel_l2_losses, test_rel_l2_losses, output_dir, 'UFNO')
...
plot_predictions(model, x_test, y_test, grid, output_dir, 'UFNO',
